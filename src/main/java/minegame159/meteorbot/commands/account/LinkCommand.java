...
public class LinkCommand extends Command {
    ...
    public void run(MessageReceivedEvent event) {
        ...
        if (user.mcAccounts.size() < user.maxMcAccounts) {
            ...
            event.getChannel().sendMessage(Utils.embed("Your %d slots are full, use `.unlink <username>` and `.info` to see your linked minecraft accounts.", user.maxMcAccounts).build()).queue();
        }
    }
}
